 ...
 * 
 * <p>
 * Code has been refactored to work with PuzzleState interface to encapsulate
 * state representation and connection between states.
 * 
 * <p>
 * For some changes:
 * 
...
public class PuzzleGameGUI extends Frame implements ActionListener, MouseListener, MouseMotionListener, ItemListener {
	// Default number of steps for the shuffle operation
	// This means the user needs to perform 15 steps (at most) to get to the initial solution
	// This means the internal sequence of states has length 15 after a shuffle operation
	private static final int DEFAULT_SHUFFLE_PATHLENGTH = 15;
	// Default pixel values for the offset to the sides of the overall window, the window frame
	private static final int DEFAULT_OFFSET = 50;
	// Default pixel values for the width and height of a single tile
	...
	// Game representation
	private PuzzleState currentState; // current state of the game
	...
	// each tile has a number value (range 1,2,...,15) that has a corresponding
	// little image
	// get/set methods encapsulate access to array
	private BufferedImage[] images ;// = new BufferedImage[15];

	// Default setting: 4x4 board with 100x100 pixel squares and a 50 pixel border
	// frame. Values controlled by DEFAULT_HEIGHT, WIDTH, OFFSET constants.
	// total dimensions: 500x500 as it is 500 = 4 x 100 + 2 x 50 for overall width
	// and height
	...
	private int emptySquares ; //= 1; initial setting
	...
	// Point is abused to store (x,y) coordinates, it is not used as an awt graphics
	// object
	private Point selectedSquare;
	private Point checkSquare;
	private boolean dontChange ; //= false; initial setting
	private boolean close ; //= true; initial setting

	/**
	 * Constructor sets up UI and default board with numbers
	 */
	public PuzzleGameGUI() {
		super("The Sliding Puzzle Game");
		// initialize private variables
		// images holds little squares of image fragments obtain from 1 large image
		images = new BufferedImage[15];
		// default setting for the number of empty tiles is 1
		emptySquares = 1;
		// some for boolean variables
		dontChange = false;
		close = true;
		...
		// set dimensions to default values and adjust the window
		setDimensions();
		resizeWindow();
		// set the game to the starting state
		currentState = new SimplePuzzleState();
		setFinalState();
		initBoard();
	}
	...
	/**
	 * Fills the menubar with pulldown menus for filehandling, organizing the board,
	 * setting visibility of lines and numbers. This object serves as the one and
	 * only listener for all menu items.
	 */
	private void setupMenuBar() {
		setMenuBar(menuBar = new MenuBar());

		// add and populate the menu bar entry named File
		...
		// add and populate the menu bar entry named Edit
		...
		// add and populate the menu bar entry named View
	...
	private void resizeWindow() {
		// it is a 4 x 4 game with the offset on left and right, resp top and bottom
	...
	/**
	 * Sets dimensions to default values. Dimensions may change if a picture is
	 * loaded from file.
	 */
	...
	//////////////////////////////////////////////////////////////////////////////////// ////////////////////////////////////////////
	/**
	 * Operates a pop up dialog to congratulate the user for solving the game.
	 */
	private void runWinningDialog() {
		// IO Labels
		// The position in the options array is used to identify the user's selection
		// so 0 means Yes, 1 means No
		Object[] options = { "Yes", "No" };
		String title = "Congrulations";
		String text = "Hooray! You won!\nWould you like to continue?";
		// show dialog and capture response, which is the index of the options array
		int n = JOptionPane.showOptionDialog(this, text, title, JOptionPane.YES_NO_CANCEL_OPTION,
	...
	/**
	 * Operates a pop up dialog to let the user select the number of empty squares
	 * for the game. The board and display are updated accordingly. Changes field
	 * emptysquares.
	 */
	private void runDialogForEmpties() {
		...
		// show dialog and capture response, which is the selected string
		String s = (String) JOptionPane.showInputDialog(empty, text, title, JOptionPane.QUESTION_MESSAGE, icon,
				possibilities, initialValue);
	...
	/**
	 * Operates a pop up dialog to let the user select a file with a jpg figure to
	 * display. The board and display are updated accordingly.
	 */
	...
	//////////////////////////////////////////////////////////////////////////////////// /////////////////////////////////////////////
	/**
	 * Loads an image from the given file and prepares a 4x4 board of images. Method
	 * also resizes and updates the screen.
	 * 
	...
	/**
	 * Split image into a 16 square fragments for a 4 x 4 board. If image is
	 * rectangular, we select the largest possible square image that is contained in
	 * the overall rectangular image.
	 * 
	...
	private void setUpImages(BufferedImage image) {
		...
		// (ox,oy) are coordinates for the top-left corner for the part of the figure we
		// want
		int ox = cx - 2 * w;
		int oy = cy - 2 * h;
		// split figure
	...
	/**
	 * For a tile with the given value, this method retrieves the corresponding
	 * image.
	 * 
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (item.equals("Open")) {
			...
			System.out.println("Current #Steps from initial state: " + currentState.getPathLength());
			...
			// shortest path
			currentState = currentState.getStateWithShortestPath();
			System.out.println("Finished calculation of shortest path");
			System.out.println("Now #Steps from initial state: " + currentState.getPathLength());
	 ...
	 * 
	 * @param e provides information for the event, e.g. if user clicked on the
	 *          frame or the board.
	 */
	@Override
	public void mouseClicked(MouseEvent e) {
		...
		if (automatic.getState()) {
			...
			PuzzleState parent = currentState.getParent();
			if (null != parent) {
	...
	/**
	 * If user drags a tile, it moves into an adjacent empty slot. This method is
			...
			if (selectedSquare != null) {
				if (checkSquare != null) {
					...
					if (isEmpty(checkSquare)) {
						PuzzleState tmp = currentState.drag(selectedSquare.y, selectedSquare.x, checkSquare.y,
								checkSquare.x);
						if (null != tmp)
							currentState = tmp;
						else
	...
	//
	/**
	 * Compute the top-left y coordinate for the current row y. Used for drawing a
	 * tile.
	 * 
	 * @param r current row
	...
	/**
	 * Compute the top-left x coordinate for the current column c. Used for drawing
	 * a tile.
	 * 
	...
	private boolean isEmpty(Point p) {
		return currentState.isEmpty(p.y, p.x);
	}
	...
	private boolean hasEmptyNeighbor(Point p) {
		// above
		if (p.y > 0 && currentState.isEmpty(p.y - 1, p.x))
			return true;
		// below
		if (p.y < 3 && currentState.isEmpty(p.y + 1, p.x))
			return true;
		// left
		if (p.x > 0 && currentState.isEmpty(p.y, p.x - 1))
			return true;
		// right
		if (p.x < 3 && currentState.isEmpty(p.y, p.x + 1))
			return true;
		return false;
	}
	...
	/**
	 * Translate row, column coordinates into an index ranging from 0,1,...,
	 * dimension^2-1
	 * 
	...
	/**
	 * Decides if a given position as pixel coordinates is on the board and not on
	 * the frame.
	 * 
	...
	private boolean isValid(Point p) {
		...
		return ((offX < p.x) && (p.x < 4 * w + offX)) && ((offY < p.y) && (p.y < 4 * h + offY));
	}

	/**
	 * Initializes or resets the field finalState. Uses field emptySquares to adjust
	 * number of empty squares for the final state.
	 */
	...
	/**
	 * Resets game to 4x4 board and a state where squares are ordered. Resets field
	 * state, uses field emptySquares to determine number of empty squares.
	 */
	private void initBoard() {
		currentState.setToInitialState(4, emptySquares);
	}

	/**
	 * Perform a sequence of random moves for tiles to start the game. Pathlength is
	 * chosen according to default path length setting.
	 */
	private void shuffleBoard() {
		// We use the default constant as path length.
		// Value can be adjusted by setting the constant as needed.
		currentState = currentState.shuffleBoard(DEFAULT_SHUFFLE_PATHLENGTH);
	}
	...
	private boolean moveOnClick(Point p) {
		...
		for (Operation op : Operation.values()) {
			ps = currentState.move(p.y, p.x, op);
			if (null != ps) {
	...
	/**
	 * Draws the lines to clearly separate tiles. Line color is black.
	 * 
	 * @param g the graphics object to draw on
	 */
	...
	private void drawContentForSquares(Graphics g) {
		...
		for (int r = 0; r <= 3; r++) {
			for (int c = 0; c <= 3; c++) {
				value = currentState.getValue(r, c);
				if (value == 0) {
	...
	public static void main(String[] args) {
		if (args != null && args.length > 0) {
			System.out.println("Warning: command line parameters not supported, values are ignored");
		}
